...
test('loadConfig: project protection arrays accumulate onto defaults; scalars last-win', () => {
  const proj = mkTmp();
  try {
    mkdirSync(join(proj, '.claude'), { recursive: true });
    writeFileSync(
      join(proj, '.claude', 'kube-guard.config.json'),
      JSON.stringify({ protectedNamespaces: ['kg-test-ns-xyz'], allowExec: true }),
    );
    const cfg = loadConfig(proj);
    // protection arrays ACCUMULATE (#8): the project's entry is added, defaults kept.
    assert.ok(cfg.protectedNamespaces.includes('kg-test-ns-xyz'));
    assert.ok(cfg.protectedNamespaces.includes('kube-system'));
    // scalar keys still last-win.
